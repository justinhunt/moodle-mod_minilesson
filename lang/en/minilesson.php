...
$string['managemcp_oauthrequirements'] = 'OAuth server requirement: OAuth delivers its access token as an "Authorization: Bearer" header, which some server setups (notably Apache with mod_proxy_fcgi/PHP-FPM, or plain mod_php on some distros) strip before PHP ever sees it. If AI clients can register and start the login flow but tool calls then fail with "Authentication required", add `CGIPassAuth On` to your Apache vhost (Apache 2.4.13+), or `SetEnvIf Authorization "(.*)" HTTP_AUTHORIZATION=$1` as a broader-compatibility alternative, and confirm with a test request that compares an X-API-Key call against an equivalent Authorization: Bearer call.';
