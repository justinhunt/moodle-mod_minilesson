...
class gradenow_renderer extends \plugin_renderer_base {
    ...
    public function render_attempt_scoresheader($gradenow){
        ...
        $errorrate = $this->render_errorratedetails();
        $scrate = $this->render_scratedetails();
        $mistakes = $this->render_mistakedetails();
        $actionheader = \html_writer::div($audio . $mistakes . $wpm . $accuracy . $errorrate . $scrate,
    ...
    public function render_machinereview_buttons($gradenow){
        ...
        $poodlltimeid = $gradenow->attemptdetails('poodlltimeid');
        $url = new \moodle_url(constants::M_URL . '/grading.php', array('action' => 'gradenow', 'n' => $poodlltimeid, 'attemptid' => $attemptid));
    ...
    public function render_markeduppassage($passage)
    {
    ...
    public function render_passage($passage){
        $usepassage = $this->render_markeduppassage($passage);
    ...
    public function render_passageactions(){
        ...
        $msvbutton = \html_writer::tag('button',
            get_string('msvbutton',constants::M_COMPONENT),
            array('type'=>'button','id'=>constants::M_CLASS .'_msvbutton','class'=>constants::M_CLASS .'_msvbutton btn btn-success'));

        $spotcheckbutton = \html_writer::tag('button',
            get_string('spotcheckbutton',constants::M_COMPONENT),
            array('type'=>'button','id'=>constants::M_CLASS .'_spotcheckbutton','class'=>constants::M_CLASS .'_spotcheckbutton btn btn-warning'));

        $transcriptcheckbutton = \html_writer::tag('button',
            get_string('transcriptcheckbutton',constants::M_COMPONENT),
            array('type'=>'button','id'=>constants::M_CLASS .'_transcriptcheckbutton','class'=>constants::M_CLASS .'_transcriptcheckbutton btn btn-secondary'));
        ...
        //$buttons =  $gradingbutton . $msvbutton . $spotcheckbutton . $transcriptcheckbutton . $clearbutton;
        $buttons =  $gradingbutton . $spotcheckbutton . $msvbutton ;

        $notes= \html_writer::tag("textarea",'',
            array('id'=>constants::M_CLASS .'_notes','class'=>constants::M_CLASS .'_notes',
                'placeholder'=>get_string('notesplaceholder',constants::M_COMPONENT)));

        $container = \html_writer::div($buttons . $notes,constants::M_CLASS . '_grading_passageactions');
        return $container;
    }
    ...
    public function render_errorratedetails(){
        global $CFG;
        $title = \html_writer::div(get_string('errorrate',constants::M_COMPONENT),'panel-heading');
        $score = \html_writer::div('-/-',constants::M_GRADING_SCORE . ' panel-body',array('id'=>constants::M_GRADING_ERRORRATE_SCORE));
        $ret = \html_writer::div($title . $score ,constants::M_GRADING_ERRORRATE_CONTAINER . ' panel panel-primary',
            array('id'=>constants::M_GRADING_ERRORRATE_CONTAINER));
        return $ret;
    }
    public function render_scratedetails(){
        global $CFG;
        $title = \html_writer::div(get_string('selfcorrectionrate',constants::M_COMPONENT),'panel-heading');
        $score = \html_writer::div('-/-',constants::M_GRADING_SCORE . ' panel-body',array('id'=>constants::M_GRADING_SCRATE_SCORE));
        $ret = \html_writer::div($title . $score ,constants::M_GRADING_SCRATE_CONTAINER . ' panel panel-primary',
            array('id'=>constants::M_GRADING_SCRATE_CONTAINER));
        return $ret;
    }
