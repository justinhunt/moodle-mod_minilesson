...
class machinegrading extends basereport
{
    ...
    public function fetch_formatted_field($field, $record, $withlinks)
    {
        ...
        switch ($field) {
            ...
            case 'totalattempts':
                ...
                if ($withlinks) {
                    $link = new \moodle_url(constants::M_URL . '/grading.php',
                        array('action' => 'machinegradingbyuser', 'n' => $record->poodlltimeid, 'userid' => $record->userid));
                    $ret = \html_writer::link($link, $ret);
                }
                break;
            ...
            case 'gradenow':
                if ($withlinks) {
                    $url = new \moodle_url(constants::M_URL . '/grading.php', array('action' => 'gradenow', 'n' => $record->poodlltimeid, 'attemptid' => $record->attemptid));
            ...
            case 'regrade':
                ...
                if ($withlinks) {
                    $link = new \moodle_url(constants::M_URL . '/grading.php', array('action' => 'regradenow', 'n' => $record->poodlltimeid, 'attemptid' => $record->attemptid));
            ...
            case 'review':
                ...
                if ($withlinks) {
                    $link = new \moodle_url(constants::M_URL . '/grading.php', array('action' => 'machinereview', 'n' => $record->poodlltimeid, 'attemptid' => $record->attemptid));
    ...
    public function process_raw_data($formdata)
    {
        ...
        $sql = "SELECT tai.id,tu.userid, tai.wpm, tai.accuracy,tu.timecreated,tai.attemptid, tai.sessionerrors," .
            " tai.sessionscore,tai.sessiontime,tai.sessionendword, tu.filename, tai.poodlltimeid,  u.firstnamephonetic," .
        "u.lastnamephonetic,u.middlename,u.alternatename,u.firstname,u.lastname  FROM {" . constants::M_AITABLE . "} tai INNER JOIN  {" . constants::M_USERTABLE . "}" .
            " tu ON tu.id =tai.attemptid AND tu.poodlltimeid=tai.poodlltimeid INNER JOIN {user} u ON tu.userid=u.id WHERE tu.poodlltimeid=?" .
        " ORDER BY u.lastnamephonetic,u.firstnamephonetic,u.lastname,u.firstname,u.middlename,u.alternatename, tai.id DESC";
        $alldata = $DB->get_records_sql($sql, array($formdata->poodlltimeid));
