...
class aigrade
{
    function __construct($attemptid, $modulecontextid=0) {
        ...
        if($this->attemptdata) {
            $this->activitydata = $DB->get_record(constants::M_TABLE, array('id' => $this->attemptdata->poodlltimeid));
    ...
    public function send_to_gradebook(){
        global $CFG;
        require_once($CFG->dirroot.'/mod/poodlltime/lib.php');
        poodlltime_update_grades($this->activitydata, $this->attemptdata->userid);
    }
   ...
   public function do_diff($debug=false){
       ...
       //we do not (yet) save error rates or self correction rates, so selfcorrections are not part of processscores
       $sc=self::fetch_selfcorrections($matches);
       $selfcorrections = json_encode($sc);
       $sccount=count((array)$sc);
       ...
       //session time
        ...
        if(!$sessiontime){
            ...
            if($this->activitydata->timelimit > 0 && !$this->activitydata->allowearlyexit ){
                ...
                if($sessiontime<1) {
                    //this is a guess now, We just don't know it. Probably should not get here really
                    $sessiontime = 60;
                }
            }
        }
        ...
        $record->selfcorrections = $selfcorrections;
        $record->sccount = $sccount;
       ...
       $this->aidata->selfcorrections = $selfcorrections;
       $this->aidata->sccount = $sccount;
    ...
    //fetch a list of guessed selfcorrections based on session matches
    public function fetch_selfcorrections($sessionmatches){
        $selfcorrections=  new \stdClass();;
        if($sessionmatches){
            $prevmatch=false;
            //loop through matches checking for insertions prior to matches
            foreach($sessionmatches as $match){
                $maybe=false; // insertions exist between this match and prev match
                $verymaybe=false; //this word is matched and prev word is matched, but insertions exist
                ...
                if($prevmatch){
                    ...
                    if($match->tposition - $prevmatch->tposition > 1){
                        ...
                        if($match->pposition - $prevmatch->pposition == 1){
                            if(isset($prevmatch->altmatch) && $prevmatch->altmatch===1) {
                                $verymaybe = false;
                            }else{
                                $verymaybe = true;
                            }
                ...
                if($verymaybe){
                    $sc = new \stdClass();
                    $sc->word=$match->word;
                    $sc->wordnumber=$match->pposition ;
                    $sc->msv=new \stdClass();
                    $selfcorrections->{$match->pposition}=$sc;
                }
                $prevmatch =$match;
            }
        }
