define(
    ...
    */
      ...
      //prepare results data for detailed review on finished page or by teacher
                var results_data = {};
                results_data.correctitems = self.items.filter(function (e) {
    ...
    //No syllables so use phonemes for our phonemes bar
                ...
                // but if we have them we can try to map them to letters.
                        ...
                        if (have_phoneme_names) {
                            var adata = self.markuphelper.alignPhonemesToLetters(
                                wordobject.Word,
                                wordobject.Phonemes,
                                twoletterlang
                            );
        ...
        //Display result of all words, with phoneme bars per word if we have them
                templates.render('minilessonitem_fluency/fluencylineresult', {wordresults: wordresults}).then(
                    function (html, js) {
                        ...
                        if (!isReview) {
                            ...
                            templates.render('minilessonitem_fluency/fluencylineresult', {wordresults: wordresults_review}).then(
    ...
    //make a chart for each score
          ...
          //Prepare items for display in the listitemresults template (here and letter in finished review)
            ...
            scoreToColorClass: function (score) {
                if (score === null) {
                    return "letter_missing"; //grey
                }
                if (score >= this.phonemeWarningThreshold) {
                    return "letter_good"; //green
                }
          ...
          // Language mappings for phoneme to grapheme groups
            ...
            // Tokenize phonemes based on mappings
                    function normalizePhoneme(p)
                    {
                        for (const group in phonemeGroups) {
                            if (phonemeGroups[group].includes(p)) {
                                return group;
                            }
                        }
                        return p;
            ...
            // Dynamic programming
                    ...
                    for (let i = 0; i <= m; i++) {
                        dp[i][0] = i * gap; traceback[i][0] = "up"; }
                    for (let j = 0; j <= n; j++) {
                        dp[0][j] = j * gap; traceback[0][j] = "left"; }
