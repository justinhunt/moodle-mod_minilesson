...
$PAGE->set_url(constants::M_URL . '/aigen.php',
        ['id' => $cm->id]);
...
if ($aigenform->is_cancelled()) {
    ...
    foreach ($tdata['items'] as $itemnumber => $item) {
        ...
        if (class_exists($itemclass)) {
            ...
            // Fetch the prompt
            $theprompt = $itemclass::aigen_fetch_prompt($theactivity, 'generate');
            $tdata['items'][$itemnumber]['aigenprompt'] = $theprompt;
            ...
            $tdata['items'][$itemnumber]['promptfields'] = utils::extract_curly_fields($theprompt);
