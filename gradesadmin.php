...
$alternatives_form = new \mod_poodlltime\form\alternatives();
...
switch($action){
    ...
    case 'machineregradeall':
        $url =  new \moodle_url(constants::M_URL . '/gradesadmin.php',
            array('id' => $cm->id,
                'action'=>'menu'));
        $ai_evals = $DB->get_records(constants::M_AITABLE,array('poodlltimeid'=>$moduleinstance->id));
        if(!$ai_evals) {
            redirect($url,get_string('noattemptsregrade',constants::M_COMPONENT));
        }else{
            $skipped=0;
            foreach($ai_evals as $eval){
                $aigrade = new \mod_poodlltime\aigrade($eval->attemptid,$modulecontext->id);
    ...
    case 'pushmachinegrades':
        ...
        if($moduleinstance->machgrademethod == constants::MACHINEGRADE_MACHINE &&
            ...
            poodlltime_update_grades($moduleinstance);
        }
...
$mform = new \mod_poodlltime\form\alternatives();
